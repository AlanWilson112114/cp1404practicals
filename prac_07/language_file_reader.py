...
def main():
    ...
    with open('languages.csv', 'r') as in_file:
        ...
        for line in in_file:
            ...
            if len(parts) < 5:
                continue  # Skip lines that are incomplete or malformed

            # reflection is stored as a string (Yes/No) and we want a Boolean
            reflection = parts[2].strip() == "Yes"
            pointer_arithmetic = parts[3].strip() == "Yes"
            ...
            try:
                ...
                print(f"Invalid year value in line: {line.strip()}")
                continue
